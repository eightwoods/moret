...
library MarketLibrary {
  ...
  function getTokenBalances(address _contractAddress, address _protocolDataProviderAddress, address _tokenAddress)
...
/*
  function getTokenAndCollateral(address _contractAddress, address _protocolDataProviderAddress, address _tokenAddress)
  public view returns(uint256, uint256)
  {
    (address _aToken,,) = getLendingTokenAddresses(_protocolDataProviderAddress, _tokenAddress);
    return  (IERC20(_aToken).balanceOf(_contractAddress), int256(IERC20(_tokenAddress).balanceOf(_contractAddress)));
  }
